...
impl Auth for AuthService {
    ...
    async fn sign_out(
        ...
        match result {
            Ok(_) => Ok(Response::new(SignOutResponse {
                status_code: StatusCode::Success.into(),
            })),
            Err(_) => Ok(Response::new(SignOutResponse {
                status_code: StatusCode::Failure.into(),
            })),
        }
    }
...
mod tests {
    ...
    async fn sign_out_should_succeed() {
        ...
        auth_service.sign_up(request).await.unwrap();
        ...
        let result: SignInResponse = auth_service.sign_in(request).await.unwrap().into_inner();

        let request = tonic::Request::new(SignOutRequest {
            user_uuid: result.user_uuid.clone(),
        });
